...
const OrdersAndPayment = () => {
    ...
    const fetchOrders = async () => {
        try {
            const response = await axios.get("http://localhost:5000/api/orders/"+cusid, {
            ...
            if (response.data.orders.length > 0) {
                const newFilter = response.data.orders.filter((order) => order.Status !== "Paid");
                setOrders(newFilter);
            } else {
    ...
    const handlePayment = async () => {
        ...
        try {
            ...
            if (response.data.status === "success") {
                ...
                await axios.put(`http://localhost:5000/api/orders/${selectedOrderID}`, { Status: "Paid" }, {
                    headers: { Authorization: `Bearer ${token}` }
                });
    ...
    return (
        ...
        >
            <h2 className="text-center text-primary fw-bold mb-4">
                📦 <span className="text-dark">Order & Payment</span>
            </h2>
                ...
                <>
                    <div className="p-3 mb-4 rounded shadow-sm bg-light">
                        ...
                        <p className="mb-1">
                            ...
                            <span className={`badge ms-2 ${orderDetails.Status === "Completed" ? "bg-success" : "bg-warning"}`}>
                                {orderDetails.Status}
                            </span>
                        </p>
                    </div>
